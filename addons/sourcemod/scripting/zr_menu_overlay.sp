...
static StringMap g_smOverlayNameH;
static StringMap g_smOverlayPathH;
static StringMap g_smOverlayNameZ;
...
int g_iOverlay_Humans = 0,
	...
	g_iOverlayClientZombie[MAXPLAYERS + 1] = {-1, ...},
...
{
	g_smOverlayNameH = new StringMap();
	g_smOverlayPathH = new StringMap();
	g_smOverlayNameZ = new StringMap();
	...
	if(KvJumpToKey(g_hKvOverlay, "Overlay_Humans"))
	{
		if(KvGotoFirstSubKey(g_hKvOverlay))
		{
			do
			{
				...
				KvGetString(g_hKvOverlay, "name", sBuffer_temp, sizeof(sBuffer_temp), "");
				g_smOverlayNameH.SetString(sIndex_temp, sBuffer_temp, true);
	...
	if(KvJumpToKey(g_hKvOverlay, "Overlay_Zombies"))
	{
		if(KvGotoFirstSubKey(g_hKvOverlay))
		{
			do
			{
				...
				KvGetString(g_hKvOverlay, "name", sBuffer_temp, sizeof(sBuffer_temp), "");
				g_smOverlayNameZ.SetString(sIndex_temp, sBuffer_temp, true);
				KvGetString(g_hKvOverlay, "Overlay", sBuffer_temp, sizeof(sBuffer_temp), "");
...
public void OnClientCookiesCached(int client)
{
	...
	if(!StrEqual(scookie, ""))
	{
		...
		if (g_iOverlayClientZombie[client] > g_iOverlay_Zombies) g_iOverlayClientZombie[client] = -1;
	}
	else	g_iOverlayClientZombie[client] = 1;

	GetClientCookie(client, g_hOverlayClientHuman, scookie, sizeof(scookie));
	if(!StrEqual(scookie, ""))
	{
		g_iOverlayClientHuman[client] = StringToInt(scookie);
		if (g_iOverlayClientHuman[client] > g_iOverlay_Humans) g_iOverlayClientHuman[client] = -1;
	}
	else	g_iOverlayClientHuman[client] = 1;
}
...
void MakeMenuOverlay(int client, int bMenu = 0, int i_Last = -1)
{
	...
	if(bMenu == 3)
	{
		...
		if (g_iOverlayClientHuman[client] == -1) Format(sbuffer, sizeof(sbuffer), "%s [X]", Menu_Disabled);
		else Format(sbuffer, sizeof(sbuffer), "%s [ ]", Menu_Disabled);
		MenuOverlay.AddItem("-1", sbuffer);

		if (g_iOverlayClientHuman[client] == 0) Format(sbuffer, sizeof(sbuffer), "%s [X]", Menu_Random);
		else Format(sbuffer, sizeof(sbuffer), "%s [ ]", Menu_Random);
		MenuOverlay.AddItem("0", sbuffer);
		
		for (int i = 1; i <= g_iOverlay_Humans; i++)
		{
			IntToString(i, sIndex_temp, sizeof(sIndex_temp));
			g_smOverlayNameH.GetString(sIndex_temp, sbuffer, sizeof(sbuffer));
	...
	else if(bMenu == 2)
	{
		...
		if (g_iOverlayClientZombie[client] == -1) Format(sbuffer, sizeof(sbuffer), "%s [X]", Menu_Disabled);
		else Format(sbuffer, sizeof(sbuffer), "%s [ ]", Menu_Disabled);
		MenuOverlay.AddItem("-1", sbuffer);

		if (g_iOverlayClientZombie[client] == 0) Format(sbuffer, sizeof(sbuffer), "%s [X]", Menu_Random);
		else Format(sbuffer, sizeof(sbuffer), "%s [ ]", Menu_Random);
		MenuOverlay.AddItem("0", sbuffer);

		for (int i = 1; i <= g_iOverlay_Zombies; i++)
		{
			IntToString(i, sIndex_temp, sizeof(sIndex_temp));
			g_smOverlayNameZ.GetString(sIndex_temp, sbuffer, sizeof(sbuffer));
	...
	else if(bMenu <= 1)
	{
		...
		if (g_iOverlayClientZombie[client] >= 1)
		{
			IntToString(g_iOverlayClientZombie[client], sIndex_temp, sizeof(sIndex_temp));
			g_smOverlayNameZ.GetString(sIndex_temp, sZbuffer, sizeof(sZbuffer));
			Format(sZbuffer, sizeof(sZbuffer), "%s [%s]", Menu_Zombis_Win, sZbuffer);
		}
		else if (g_iOverlayClientZombie[client] == 0) Format(sZbuffer, sizeof(sZbuffer), "%s [%s]", Menu_Zombis_Win, Menu_Random);
		else if (g_iOverlayClientZombie[client] == -1) Format(sZbuffer, sizeof(sZbuffer), "%s [%s]", Menu_Zombis_Win, Menu_Disabled);

		if (g_iOverlayClientHuman[client] >= 1)
		{
			IntToString(g_iOverlayClientHuman[client], sIndex_temp, sizeof(sIndex_temp));
			g_smOverlayNameH.GetString(sIndex_temp, sHbuffer, sizeof(sHbuffer));
			Format(sHbuffer, sizeof(sHbuffer), "%s [%s]", Menu_Humans_Win, sHbuffer);
		}
		if (g_iOverlayClientHuman[client] == 0) Format(sHbuffer, sizeof(sHbuffer), "%s [%s]", Menu_Humans_Win, Menu_Random);
		if (g_iOverlayClientHuman[client] == -1) Format(sHbuffer, sizeof(sHbuffer), "%s [%s]", Menu_Humans_Win, Menu_Disabled);
...
void PreviewOverlay(int iClient, int iTeam, int iIndex)
{
	if (IsValidClient(iClient))
	{
		...
		char sBuffer_temp[PLATFORM_MAX_PATH];
		char sIndex_temp[20];
		IntToString(iIndex, sIndex_temp, sizeof(sIndex_temp));
		...
		case CS_TEAM_T:
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && !IsFakeClient(i))
				{
					...
					{
						IntToString(g_iOverlayClientZombie[i], sIndex_temp, sizeof(sIndex_temp));
						g_smOverlayPathZ.GetString(sIndex_temp, sBuffer_temp, sizeof(sBuffer_temp));
						ShowOverlayToClient(i, sBuffer_temp);
					}
				}
		...
		case CS_TEAM_CT:
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && !IsFakeClient(i))
				{
					...
					{
						IntToString(g_iOverlayClientHuman[i], sIndex_temp, sizeof(sIndex_temp));
						g_smOverlayPathH.GetString(sIndex_temp, sBuffer_temp, sizeof(sBuffer_temp));
						ShowOverlayToClient(i, sBuffer_temp);
					}
				}
